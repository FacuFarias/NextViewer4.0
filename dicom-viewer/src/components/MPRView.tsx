...
const MPRView: React.FC<MPRViewProps> = ({
  ...
  const initMPR = useCallback(async () => {
    ...
    try {
      ...
      const volumeId = `cornerstoneStreamingImageVolume:mpr-${series.seriesInstanceUID}`;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (resizeFrameRef.current !== null) window.cancelAnimationFrame(resizeFrameRef.current);
      try {
        cache.removeVolumeLoadObject(`cornerstoneStreamingImageVolume:mpr-${series.seriesInstanceUID}`);
      } catch {
        // Ignore
      }
    };
  }, [initMPR, series.seriesInstanceUID]);
  ...
  if (embedded) {
    return (
      <div ref={containerRef} className="mpr-view-embedded">
        {error && (
  ...
  return (
    <div ref={containerRef} className="mpr-view">
